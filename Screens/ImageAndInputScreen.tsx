import { ActivityIndicator, ImageSourcePropType, KeyboardAvoidingView, Text, TextInput, TextInputProps, TouchableOpacity, TouchableWithoutFeedback, View } from 'react-native'
...
interface ImageAndInputScreenPropsTypes {
  ...
  handleChangeCredentials: (field: keyof ImageAndInputScreenPropsTypes["credentials"], value: string) => void,
...
type SignInNavigationProps = NativeStackNavigationProp<RootStackParamList, "SignIn">;

export const SingleInput = ({ icon, onChangeText, value, placeholder, setshowpswrd, showpswrd, ...rest }: SingleInputProps) => {
  return <View style={styles.singleinput}>
    ...
    <TextInput
      ...
      {...rest}
    />
    {placeholder == "Password" && setshowpswrd && <TouchableOpacity onPress={() => setshowpswrd(p => !p)} style={styles.eyeiconContainer}>
...
export const BottomTextComponent = ({ text, onPress, screentonavigate }: BottomTextComponentProps) => {
  return <View>

    <Text style={styles.alrdyacnttext}>{text}<TouchableWithoutFeedback
      onPress={onPress}
    ><Text style={styles.signintext}>{screentonavigate}</Text></TouchableWithoutFeedback></Text>
  </View>
}


const ImageAndInputScreen = ({ toptext, btntext, handleActionofButtonClick, credentials, handleChangeCredentials, topimage }: ImageAndInputScreenPropsTypes) => {
  ...
  return (
    ...
    <View style={styles.container}>
      ...
      <KeyboardAvoidingView behavior='padding' style={{ flex: 1 }}>
        <View style={{ flex: 1, paddingTop: isKeyboardVisible ? widthPercentageToDP(15) : 0 }}>
          ...
          <View style={[styles.imgandinputcontainer]}>
            ...
            <View style={[styles.inptcontainer, { justifyContent: isKeyboardVisible ? "center" : "flex-start" }]}>
              ...
              />
              {route.name == "SignIn" && <View style={{ width: "95%" }}>
                <TouchableOpacity>
                  <Text style={styles.frgtpswrdstyles}>Forgot Passowrd ?</Text>
                </TouchableOpacity>
              </View>}
              ...
              {route.name == "SignUp" ? <BottomTextComponent
